        ...
        <footer className="footer">
            ...
            <section className="footer__sitemap">
              {linkCategories.map((i, idx) => {
                return (
                  <div key={idx} className={`footer__sitemap-column ${isMobile && !i.visible ? 'footer__sitemap-column--collapsed' : ''}`}>
                    <Typography variant="subtitle1" component="h4" className={isMobile ? 'footer__sitemap-column-title--mobile': ''} onClick={() => {
                      if(!isMobile) {
                        return;
                      }
                      const newLinkState = [...linkCategories];
                      ...
                      {isMobile && i.visible ? 
                       ...
                       isMobile && !i.visible ?  <img src="/images/ic-anyafinn-plus.svg" alt="plus" />
                        : null}{i.title}</Typography>
                    {(i.visible || !isMobile ) ? 
                    <ul>
                      {i.links.map((j, idx2) => {
                        return (
                        <li key={idx2}>
                          <Typography variant="body2" component="a" href="#">{j}</Typography>
                        </li>
                        )
                      })}
                    </ul> : null}
...
export default withConfig('footer')(withStyles(styles)(Footer));